...
XUPProjectManager.XUPProjectManager( QWidget* parent )
    ...
    connect( tvFiltered.selectionModel(), SIGNAL( currentChanged(  QModelIndex&,  QModelIndex& ) ), self, SLOT( tvFiltered_currentChanged(  QModelIndex&,  QModelIndex& ) ) )
    connect( tvFiltered, SIGNAL( customContextMenuRequested(  QPoint& ) ), self, SIGNAL( projectCustomContextMenuRequested(  QPoint& ) ) )
