...
def storageToString( Settings.StoragePath type ):
    switch ( type )
    case SP_PLUGINS:
...
def storagePaths( Settings.StoragePath type ):
    ...
    if  type == SP_TEMPLATES and contains( "Templates/DefaultDirectories" ) :
    ...
    elif  type == SP_TRANSLATIONS and contains( "Translations/Path" ) :
        settings.setStoragePaths( type, value( "Translations/Path" ).toStringList() )
        settings.remove( "Translations/Path" )

    elif  type == SP_PLUGINS and contains( "Plugins/Path" ) :
...
def setStoragePaths( Settings.StoragePath type, paths ):
    setValue( QString( "Paths/%1" ).arg( storageToString( type ) ), paths )


def homeFilePath(  QString& filePath ):
...
def homePath( Settings.StoragePath type ):
...
def storagePathsOutOfBox( Settings.StoragePath type, appPath ):
    basePath = appPath
...
def setDefaultCppSyntaxHighlight():
...
def setDefaultLexerProperties(  QFont& defaultFont, write ):
    for language in pMonkeyStudio.availableLanguages():
        lexer = pMonkeyStudio.lexerForLanguage( language )
        ...
        if  write :
            lexer.writeSettings( *self, pMonkeyStudio.scintillaSettingsPath().toLocal8Bit().constData() )
