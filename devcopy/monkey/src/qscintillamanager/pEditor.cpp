...
pEditor.pEditor( QWidget* p )
    ...
    connect( QApplication.clipboard(), SIGNAL( dataChanged() ), self, SLOT( clipboardDataChanged() ) )
...
#else:
    if  not mSearchState.inProgress :
        return False
...
def search(self):
    ...
    if  pos == -1 and mSearchState.wrap :
        if  mSearchState.forward :
    ...
    if  mSearchState.show :
        startLine = SendScintilla( SCI_LINEFROMPOSITION, targstart )
    ...
    elif  ( mSearchState.startpos = targstart -1 ) < 0 :
        mSearchState.startpos = 0
...
def simpleSearch(self):
    ...
    if  pos != -1 :
         start = from +text.left( pos ).toUtf8().length()
