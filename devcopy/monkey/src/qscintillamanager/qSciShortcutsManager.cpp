...
qSciShortcutsManager.qSciShortcutsManager (QObject* parent): QObject(parent)
    ...
    for sact in sactions:
        qact = MonkeyCore.menuBar().action( sact.name, sact.text, sact.icon, sact.defaultShortcut, sact.toolTip )
        qact.setData( sact.messageCode )
        qact.triggered.connect(self.keyBoardShortcutPressed)



void qSciShortcutsManager.keyBoardShortcutPressed ()
    Q_ASSERT( sender() )
    messageCode = qobject_cast<QAction*>( sender() ).data().toInt()
    Q_ASSERT( messageCode )
    child = MonkeyCore.workspace().currentDocument()
    if  child :
        editor = child.editor()
        if  editor and ( editor.hasFocus() or child.isWindow() ) :
             mCursorPos = editor.cursorPosition()
            ...
            elif  messageCode == QsciScintilla.SCI_MARKERDELETEALL :
                editor.markerDeleteAll( pEditor.mdBookmark )
            elif  messageCode == QsciScintilla.SCI_MARKERPREVIOUS :
                editor.setCursorPosition( editor.markerFindPrevious( mCursorPos.y() -1, pEditor.mdBookmark ), 0 )
            elif  messageCode == QsciScintilla.SCI_MARKERNEXT :
