...
def addParser(self, p ):
    ...
    mParsers[p.name()] = p
    p.newStepAvailable.connect(self.newStepAvailable)
    p.newStepsAvailable.connect(self.newStepsAvailable)
...
def removeParser(self, p ):
    if  p and mParsers.contains( p.name() ) :
        disp.newStepAvailable.connect(self.newStepAvailable)
        disp.newStepsAvailable.connect(self.newStepsAvailable)
