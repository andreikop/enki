...
public:
    ...
    QString command()  { return mCommand;
    QString arguments()  { return mArguments;
    QString workingDirectory()  { return mWorkingDirectory;
    QStringList parsers()  { return mParsers;
    bool skipOnError()  { return mSkipOnError;
    bool tryAllParsers()  { return mTryAllParsers;
    QVariant userData()  { return mUserData;
    XUPProjectItem* project()  { return mProject;
    pCommandTargetExecution& targetExecution() { return mTargetExecution;

    void setText(  QString& s ) { mText = s;
    void setCommand(  QString& s ) { mCommand = s;
    void setArguments(  QString& s ) { mArguments = s;
    void setWorkingDirectory(  QString& s ) { mWorkingDirectory = s;
    void addParser(  QString& p ) { if ( not mParsers.contains( p ) ) mParsers << p;
    void setParsers(  QStringList& p ) { mParsers = p;
    void addParsers(  QStringList& p ) { for s in p: addParser( s );
    void setSkipOnError( bool b ) { mSkipOnError = b;
    void setTryAllParsers( bool b ) { mTryAllParsers = b;
    void setUserData(  QVariant& data ) { mUserData = data;
