...
public:
    ...
    pOpenedFileExplorer* dockWidget()
    QFileSystemWatcher* fileWatcher()
    
    pAbstractChild* document( int index )
    int indexOfDocument( pAbstractChild* document )
    QList<pAbstractChild*> documents()
    
    void setCurrentDocument( pAbstractChild* document )
    pAbstractChild* currentDocument()
    
    void goToLine(  QString& fileName, pos, codec, selectionLength = -1 )
    void closeDocument( pAbstractChild* document, showDialog = True )
    pWorkspace.ViewMode documentMode()
...
protected slots:
    ...
    void internal_urlsDropped(  QList<QUrl>& urls )
    void internal_currentProjectChanged( XUPProjectItem* currentProject, previousProject )
    void internal_projectInstallCommandRequested(  pCommand& cmd, mnu )
    void internal_projectUninstallCommandRequested(  pCommand& cmd, mnu )
