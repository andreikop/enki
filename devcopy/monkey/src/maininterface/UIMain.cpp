...
def initConnections(self):
    ...
    connect( MonkeyCore.recentsManager(), SIGNAL( openFileRequested(  QString&,  QString& ) ), MonkeyCore.fileManager(), SLOT( openFile(  QString&,  QString& ) ) )
    ...
    # docks
    connect( menuBar().menu( "mDocks" ), SIGNAL( aboutToShow() ), self, SLOT( menu_Docks_aboutToShow() ) )
    # project connection
    connect( MonkeyCore.recentsManager(), SIGNAL( openProjectRequested(  QString&,  QString& ) ), MonkeyCore.projectsManager(), SLOT( openProject(  QString&,  QString& ) ) )
    connect( MonkeyCore.projectsManager(), SIGNAL( fileDoubleClicked(  QString&,  QString& ) ), MonkeyCore.workspace(), SLOT( openFile(  QString&,  QString& ) ) )
    # builder debugger interpreter menu
    connect( menuBar().menu( "mBuilder" ), SIGNAL( aboutToShow() ), self, SLOT( menu_CustomAction_aboutToShow() ) )
    connect( menuBar().menu( "mDebugger" ), SIGNAL( aboutToShow() ), self, SLOT( menu_CustomAction_aboutToShow() ) )
    connect( menuBar().menu( "mInterpreter" ), SIGNAL( aboutToShow() ), self, SLOT( menu_CustomAction_aboutToShow() ) )
    # plugins menu
    # window menu
