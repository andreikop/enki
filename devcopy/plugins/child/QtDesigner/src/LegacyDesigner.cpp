...
def defaultPluginPaths(self):
    ...
    foreach ( QString &path, path_list)        libPath = path
...
def showPreview(self, fw, style, errorMessage ):
    ...
    '''
        ...
        QObject.fw.destroyed.connect(widget.close)
            ...
            QObject.fw.core().formWindowManager().activeFormWindowChanged.connect(widget.close)
        #break
    #
    ...
    if firstPreview:        widget.move(fw.mapToGlobal(QPoint(Spacing, Spacing)))
    } else:
    '''
