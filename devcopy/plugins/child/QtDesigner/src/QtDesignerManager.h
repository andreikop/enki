...
namespace qdesigner_internal    class QDesignerIntegration
...
public:
    QtDesignerManager( parent = 0 )
    virtual ~QtDesignerManager()

    QDesignerFormEditorInterface* core()
    inline qdesigner_internal.QDesignerIntegration* integration()  { return mIntegration;
    inline QAction* editWidgetsAction()  { return aEditWidgets;
    inline QWidgetAction* previewFormAction()  { return aPreview;
    inline QList<QAction*> modesActions()  { return aModes.actions();

    QDesignerFormWindowInterface* createNewForm( parent = 0 )

    void addFormWindow( QDesignerFormWindowInterface* form )
    void setActiveFormWindow( QDesignerFormWindowInterface* form )

    QWidget* previewWidget( QDesignerFormWindowInterface* form, style = QString.null )
    QPixmap previewPixmap( QDesignerFormWindowInterface* form, style = QString.null )
...
protected:
    ...
    void setToolBarsIconSize(  QSize& size )
    void updateMacAttributes()

protected slots:
    void editWidgets()
    void previewCurrentForm(  style = QString.null )
