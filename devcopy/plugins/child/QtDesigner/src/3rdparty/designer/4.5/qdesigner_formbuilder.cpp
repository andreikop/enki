...
static QString summarizeScriptErrors( QFormScriptRunner.Errors &errors)
    ...
    foreach (QFormScriptRunner.Error error, errors)        rc += QLatin1Char('\n')
...
QDesignerFormBuilder.QDesignerFormBuilder(QDesignerFormEditorInterface *core,
    ...
    switch (m_mode)    case DisableScripts:
...
QWidget *QDesignerFormBuilder.createWidget( QString &widgetName, *parentWidget, &name)
    ...
    if widgetName == QLatin1String("QToolBar"):        widget = QToolBar(parentWidget)
    } elif widgetName == QLatin1String("QMenu"):        widget = QMenu(parentWidget)
    } elif widgetName == QLatin1String("QMenuBar"):        widget = QMenuBar(parentWidget)
    } else:
...
def addItem(self, *ui_widget, *widget, *parentWidget):
    ...
    if QDesignerContainerExtension *container = qt_extension<QDesignerContainerExtension*>(m_core.extensionManager(), parentWidget):        container.addWidget(widget)
        return True
...
static bool readDomEnumerationValue( DomProperty *p,
                                    ...
                                    QVariant &v)
    switch (p.kind())    case DomProperty.Set:         index = sheet.indexOf(p.attributeName())
        if index == -1:
            return False
         sheetValue = sheet.property(index)
        if qVariantCanConvert<PropertySheetFlagValue>(sheetValue):             f = qvariant_cast<PropertySheetFlagValue>(sheetValue)
            ok = False
        ...
        break
    case DomProperty.Enum:         index = sheet.indexOf(p.attributeName())
        if index == -1:
            return False
         sheetValue = sheet.property(index)
        if qVariantCanConvert<PropertySheetEnumValue>(sheetValue):             e = qvariant_cast<PropertySheetEnumValue>(sheetValue)
            ok = False
...
def applyProperties(self, *o, &properties):
    ...
    if designerPropertySheet:        if designerPropertySheet.pixmapCache():
    ...
    for (it = properties.constBegin(); it != cend; ++it)        DomProperty *p = *it
        QVariant v
        ...
        if not dynamicPropertiesAllowed:            if (changingMetaObject) # Changes after setting control of QAxWidget
...
def createResources(self, *resources):
    if m_ignoreCreateResources:
        return
    QStringList paths
    if resources != 0:         QList<DomResource*> dom_include = resources.elementInclude()
        foreach (DomResource *res, dom_include)            path = QDir.cleanPath(workingDirectory().absoluteFilePath(res.attributeLocation()))
            paths << path
...
QWidget *QDesignerFormBuilder.createPreview( QDesignerFormWindowInterface *fw,
    ...
    if not styleToUse.isEmpty():        if WidgetFactory *wf = qobject_cast<qdesigner_internal.WidgetFactory *>(fw.core().widgetFactory()):            if styleToUse != wf.styleName():
                WidgetFactory.applyStyleToTopLevel(wf.getStyle(styleToUse), widget)


    # Check for script errors
    *scriptErrors = builder.formScriptRunner().errors()
    if not scriptErrors.empty():        *errorMessage = summarizeScriptErrors(*scriptErrors)
        delete widget
        return  0

    # Fake application style sheet by prepending. (If self doesn't work, by nesting
    # into parent widget).
    if not appStyleSheet.isEmpty():        styleSheet = appStyleSheet
...
QWidget *QDesignerFormBuilder.createPreview( QDesignerFormWindowInterface *fw, &styleName, &appStyleSheet)
    ...
    if not widget:        # Display Script errors or message box
        QWidget *dialogParent = fw.core().topLevel()
        if scriptErrors.empty():            fw.core().dialogGui().message(dialogParent, QDesignerDialogGuiInterface.PreviewFailureMessage,
