...
FormResizer.FormResizer(QWidget *parent) :
    ...
    for (i = SizeHandleRect.LeftTop; i <= SizeHandleRect.Left; ++i)        SizeHandleRect *shr = SizeHandleRect(self, static_cast<SizeHandleRect.Direction>(i), self)
        shr.mouseButtonReleased.connect(self.formWindowSizeChanged)
...
def updateGeometry(self):
    ...
    if debugFormResizer:
     ...
     hend =  m_handles.end()
    for (it = m_handles.begin(); it != hend; ++it)        SizeHandleRect *hndl = *it;
        switch (hndl.dir())        case SizeHandleRect.LeftTop:
...
def update(self):
     hend =  m_handles.end()
    for (it = m_handles.begin(); it != hend; ++it)        (*it).update()
...
def mainContainerChanged(self):
    ...
    if  QWidget *mc = mainContainer():        # Set Maximum size which is not handled via a hint (as opposed to minimum size)
         maxWidgetSize = QSize(QWIDGETSIZE_MAX, QWIDGETSIZE_MAX)
