...
QtAssistantDock.QtAssistantDock( QWidget* parent )
    ...
    # connections
    connect( aShow, SIGNAL( triggered() ), self, SIGNAL( helpShown() ) )
    ...
    connect( mHelpEngine.contentWidget(), SIGNAL( linkActivated(  QUrl& ) ), self, SLOT( openUrl(  QUrl& ) ) )
    connect( mHelpEngine.indexWidget(), SIGNAL( linkActivated(  QUrl&,  QString& ) ), self, SLOT( openUrl(  QUrl& ) ) )
    connect( mHelpEngine.indexWidget(), SIGNAL( linksActivated(  QMap<QString, QUrl>&,  QString& ) ), self, SLOT( openUrls(  QMap<QString, QUrl>&,  QString& ) ) )
    connect( mHelpEngine.indexModel(), SIGNAL( indexCreationStarted() ),self, SLOT( disableSearchLineEdit() ) )
    connect( mHelpEngine.indexModel(), SIGNAL( indexCreated() ),self, SLOT( enableSearchLineEdit() ) )
    ...
    connect( mHelpEngine.searchEngine(), SIGNAL( searchingStarted() ), self, SLOT( searchingStarted() ) )
    connect( mHelpEngine.searchEngine(), SIGNAL( searchingFinished( int ) ), self, SLOT( searchingFinished( int ) ) )
    connect( mHelpEngine.searchEngine().queryWidget(), SIGNAL( search() ), self, SLOT( search() ) )
    connect( mHelpEngine.searchEngine().resultWidget(), SIGNAL( requestShowLink(  QUrl& ) ), self, SLOT( openUrl(  QUrl& ) ) )
    connect( mHelpEngine.contentWidget(), SIGNAL( customContextMenuRequested(  QPoint& ) ), self, SLOT( open_customContextMenuRequested(  QPoint& ) ) )
    connect( mHelpEngine.indexWidget(), SIGNAL( customContextMenuRequested(  QPoint& ) ), self, SLOT( open_customContextMenuRequested(  QPoint& ) ) )
