...
PreferencesDialog.PreferencesDialog(QHelpEngineCore *helpEngine, *parent)
    ...
    if not m_hideFiltersTab:        m_ui.attributeWidget.header().hide()
    ...
    if not m_hideDocsTab:        connect(m_ui.docAddButton, SIGNAL(clicked()), self,
...
PreferencesDialog.~PreferencesDialog()
    QLatin1String key("")
    if m_appFontChanged:        key = QLatin1String("appFont")
    ...
    if m_browserFontChanged:        key = QLatin1String("browserFont")
    ...
    if m_appFontChanged or m_browserFontChanged:        updateApplicationFont.emit()
...
def updateFilterPage(self):
    ...
    foreach ( QString &filter, filters)        atts = help.filterAttributes(filter)
...
def updateAttributes(self, *item):
    ...
    QTreeWidgetItem *itm
    for (i = 0; i < m_ui.attributeWidget.topLevelItemCount(); ++i)        itm = m_ui.attributeWidget.topLevelItem(i)
...
def addFilter(self):
    ...
    if not m_filterMap.contains(filterName):        m_filterMap.insert(filterName, QStringList())
...
def addDocumentationLocal(self):
    ...
    foreach ( QString &fileName, fileNames)         nameSpace = QHelpEngineCore.namespaceName(fileName)
        if nameSpace.isEmpty():            invalidFiles.append(fileName)
            continue


        if (m_ui.registeredDocsListWidget.findItems(nameSpace,
            Qt.MatchFixedString).count())                alreadyRegistered.append(nameSpace)
                continue
    ...
    if not invalidFiles.isEmpty() or not alreadyRegistered.isEmpty():        QString message
        if not alreadyRegistered.isEmpty():            foreach ( QString &ns, alreadyRegistered)                message += tr("The namespace %1 is already registerednot ")
        ...
        if not invalidFiles.isEmpty():            message += tr("The specified file is not a valid Qt Help Filenot ")
...
def removeDocumentation(self):
    ...
    for item in l:         ns = item.text()
        if not foundBefore and values.contains(ns):            if (0 == QMessageBox.information(self, tr("Remove Documentation"),
    ...
    if m_ui.registeredDocsListWidget.count():        m_ui.registeredDocsListWidget.setCurrentRow(0,
...
def applyChanges(self):
    ...
    if not m_hideFiltersTab:        if m_filterMap.count() != m_filterMapBackup.count():            filtersWereChanged = True
        } else:
            QMapIterator<QString, it(m_filterMapBackup)
            while (it.hasNext() and not filtersWereChanged)                it.next()
                if not m_filterMap.contains(it.key()):                    filtersWereChanged = True
                } else:
                    a = it.value()
                    b = m_filterMap.value(it.key())
                    if a.count() != b.count():                        filtersWereChanged = True
                    } else:
                        QStringList.const_iterator i(a.constBegin())
                        while (i != a.constEnd())                            if not b.contains(*i):                                filtersWereChanged = True
                                break
    ...
    if filtersWereChanged:        foreach ( QString &filter, m_removedFilters)
    ...
    '''
    if m_unregDocs.count():        foreach ( QString &doc, m_unregDocs)
...
def updateFontSettingsPage(self):
    ...
    for box in allCombos:        connect(box, SIGNAL(currentIndexChanged(int)), self,
...
def updateOptionsPage(self):
    ...
    if homepage.isEmpty():        homepage = m_helpEngine.customValue(QLatin1String("defaultHomepage"),
    ...
    m_ui.blankPageButton.clicked.connect(self.setBlankPage)
    m_ui.currentPageButton.clicked.connect(self.setCurrentPage)
    m_ui.defaultPageButton.clicked.connect(self.setDefaultPage)
