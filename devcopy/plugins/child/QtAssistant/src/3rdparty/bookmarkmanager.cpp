...
BookmarkDialog.BookmarkDialog(BookmarkManager *manager, &title,
    ...
    ui.buttonBox.accepted.connect(self.addAccepted)
    ui.newFolderButton.clicked.connect(self.addNewFolder)
    ui.toolButton.clicked.connect(self.toolButtonClicked)
...
def addNewFolder(self):
    ...
    if newFolder.isValid():        ui.treeView.expand(index)
...
def toolButtonClicked(self):
    ...
    if visible:        resize(QSize(width(), 400))
...
def itemChanged(self, *item):
    if renameItem != item:        renameItem = item
        oldText = item.text()
        return


    if item.text() != oldText:        ui.bookmarkFolders.clear()
        ui.bookmarkFolders.addItems(bookmarkManager.bookmarkFolders())
...
def selectBookmarkFolder(self, &folderName):
    ...
    if folderName == tr("Bookmarks"):        ui.treeView.clearSelection()
        return
    ...
    if not list.isEmpty():         QModelIndex &index = model.indexFromItem(list.at(0))
        QItemSelectionModel *model = ui.treeView.selectionModel()
        if model:            model.setCurrentIndex(proxyModel.mapFromSource(index),
...
def customContextMenuRequested(self, &point):
    ...
    if picked == removeItem:        bookmarkManager.removeBookmarkItem(ui.treeView, proxyIndex)
        ui.bookmarkFolders.clear()
        ui.bookmarkFolders.addItems(bookmarkManager.bookmarkFolders())
    ...
    elif picked == renameItem:        BookmarkModel *model = bookmarkManager.treeBookmarkModel()
        if QStandardItem *item = model.itemFromIndex(proxyIndex):            item.setEditable(True)
...
def eventFilter(self, *object, *e):
    if object == self and e.type() == QEvent.KeyPress:        QKeyEvent *ke = static_cast<QKeyEvent*>(e)

        index = ui.treeView.currentIndex()
        switch (ke.key())            case Qt.Key_F2:                 QModelIndex &source = proxyModel.mapToSource(index)
                QStandardItem *item =
            ...
            case Qt.Key_Delete:                bookmarkManager.removeBookmarkItem(ui.treeView,
...
def removeClicked(self):
    ...
    if searchField.text().isEmpty():        bookmarkManager.removeBookmarkItem(treeView,
...
def filterChanged(self):
    ...
    if not searchBookmarks:        regExp.setPattern(searchField.text())
...
def customContextMenuRequested(self, &point):
    ...
    QMenu menu(QLatin1String(""), self)
    data = index.data(Qt.UserRole + 10).toString()
    if data == QLatin1String("Folder"):        removeItem = menu.addAction(tr("Delete Folder"))
        ...
        if searchField.text().isEmpty():            menu.addSeparator()
    ...
    if pickedAction == showItem:        requestShowLink.emit(data)

    elif pickedAction == showItemNewTab:        #CentralWidget.instance().setSourceInNewTab(data)
        requestShowLinkInNewTab.emit(data)

    elif pickedAction == removeItem:        bookmarkManager.removeBookmarkItem(treeView,
            filterBookmarkModel.mapToSource(index))

    elif pickedAction == renameItem:         QModelIndex &source = filterBookmarkModel.mapToSource(index)
        QStandardItem *item =
            bookmarkManager.treeBookmarkModel().itemFromIndex(source)
        if item:            item.setEditable(True)
            treeView.edit(index)
...
#   define SYSTEM "win"
    ...
    if showButtons:        QLayout *hlayout = QHBoxLayout()
        ...
        addButton.clicked.connect(self.addBookmark)
...
def expandItems(self):
    ...
    foreach ( QStandardItem *item, list)         QModelIndex &index = model.indexFromItem(item)
...
def focusInEvent(self, *e):
    if e.reason() != Qt.MouseFocusReason:        searchField.selectAll()
...
def eventFilter(self, *object, *e):
    if (object == self) or (object == treeView.viewport()):        index = treeView.currentIndex()
        if e.type() == QEvent.KeyPress:            QKeyEvent *ke = static_cast<QKeyEvent*>(e)
            if index.isValid() and searchField.text().isEmpty():                 QModelIndex &src = filterBookmarkModel.mapToSource(index)
                if ke.key() == Qt.Key_F2:                    QStandardItem *item =
                        bookmarkManager.treeBookmarkModel().itemFromIndex(src)
                    if item:                        item.setEditable(True)
                        treeView.edit(index)
                        item.setEditable(False)

                } elif ke.key() == Qt.Key_Delete:                    bookmarkManager.removeBookmarkItem(treeView, src)



            switch (ke.key())                default: break
                ...
                case Qt.Key_Enter:                case Qt.Key_Return:
                    ...
                    if index.isValid():                        data = index.data(Qt.UserRole + 10).toString()
                        if not data.isEmpty() and data != QLatin1String("Folder"):
                            requestShowLink.emit(data)

                }   break

                case Qt.Key_Escape:                    escapePressed.emit()
                }   break

        } elif e.type() == QEvent.MouseButtonRelease:            if index.isValid():                QMouseEvent *me = static_cast<QMouseEvent*>(e)
                controlPressed = me.modifiers() & Qt.ControlModifier
                if ((me.button() == Qt.LeftButton) and controlPressed:
                    or (me.button() == Qt.MidButton))                        data = index.data(Qt.UserRole + 10).toString()
                        if not data.isEmpty() and data != QLatin1String("Folder"):
...
def bookmarkFolders(self):
    ...
    QString data
    foreach ( QStandardItem *item, list)        data = item.data(Qt.UserRole + 10).toString()
        if data == QLatin1String("Folder"):
...
def addNewFolder(self, &index):
    ...
    if index.isValid():        treeModel.itemFromIndex(index).appendRow(item)
    } else:
...
void BookmarkManager.removeBookmarkItem(QTreeView *treeView,
    ...
    if item:        data = index.data(Qt.UserRole + 10).toString()
        if data == QLatin1String("Folder") and item.rowCount() > 0:            value = QMessageBox.question(treeView, tr("Remove"),
        ...
        if data != QLatin1String("Folder"):            QList<QStandardItem*>itemList = listModel.findItems(item.text())
            foreach ( QStandardItem *i, itemList)                if i.data(Qt.UserRole + 10) == data:                    listModel.removeRow(i.row())
                    break
...
def fillBookmarkMenu(self, *menu, *root):
    for (i = 0; i < root.rowCount(); ++i)        QStandardItem *item = root.child(i)
        if item and item.data(Qt.UserRole + 10:
            .toString() == QLatin1String("Folder"))                newMenu = menu.addMenu(folderIcon, item.text())
                if item.rowCount() > 0:
...
def urlForAction(self, action):
    if map.contains(action):         QModelIndex &index = map.value(action)
...
def itemChanged(self, *item):
    ...
    if item.text() != oldText:        if item.data(Qt.UserRole + 10).toString() != QLatin1String("Folder"):            QList<QStandardItem*>itemList = listModel.findItems(oldText)
            if itemList.count() > 0:
...
def setupBookmarkModels(self):
    ...
    while (not stream.atEnd())        stream >> depth >> name >> type >> expanded

        QStandardItem *item = QStandardItem(name)
        item.setEditable(False)
        item.setData(type, Qt.UserRole + 10)
        item.setData(expanded, Qt.UserRole + 11)
        if depth == 0:            parents.clear(); lastDepths.clear()
            treeModel.appendRow(item)
            parents << item; lastDepths << depth
        } else:
            if depth <= lastDepths.last():                while (depth <= lastDepths.last() and parents.count() > 0)                    parents.pop_back(); lastDepths.pop_back()


            parents.last().appendRow(item)
            if type == QLatin1String("Folder"):                parents << item; lastDepths << depth
...
def uniqueFolderName(self):
    ...
    if not list.isEmpty():        QStringList names
        foreach ( QStandardItem *item, list)
            names << item.text()

        for (i = 1; i <= names.count(); ++i)            folderName = (tr("New Folder") + QLatin1String(" %1")).arg(i)
...
def removeBookmarkFolderItems(self, *item):
    for (j = 0; j < item.rowCount(); ++j)        QStandardItem *child = item.child(j)
        if child.rowCount() > 0:
