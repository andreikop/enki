...
InstallDialog.InstallDialog(QHelpEngineCore *helpEngine, *parent,
    ...
    connect(m_ui.installButton, SIGNAL(clicked()), self, SLOT(install()))
    connect(m_ui.cancelButton, SIGNAL(clicked()), self, SLOT(cancelDownload()))
    connect(m_ui.browseButton, SIGNAL(clicked()), self, SLOT(browseDirectories()))
...
def updateDocItemList(self):
    ...
    for (int i=0; i<m_ui.listWidget.count(); ++i)
        ...
        if not ns.isEmpty() and registeredDocs.contains(ns):
...
def downloadNextFile(self):
    ...
    if not m_file.open(QIODevice.WriteOnly|QIODevice.Truncate):
...
def httpRequestFinished(self, requestId, error):
    if requestId == m_docInfoId  and m_buffer:
        ...
        elif not m_httpAborted:
            registeredDocs = m_helpEngine.registeredDocumentations()
            m_buffer.seek(0)
            while (m_buffer.canReadLine())
                ba = m_buffer.readLine()
        ...
        if not m_httpAborted:
            ...
            elif checkSum.isEmpty() or m_currentCheckSum != checkSum:
                m_file.remove()
                QMessageBox.warning(self, m_windowTitle,
