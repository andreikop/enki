...
ToolsManager.ToolsManager( QObject* p )
        : QObject( p )
    if  not mIconProvider :
        mIconProvider = QFileIconProvider()
...
def tools(self, type ):
    ...
    foreach (  ToolsManager.Tool& tool, mTools )
        if  ( tool.desktopEntry and type == ToolsManager.DesktopEntry :
                or ( not tool.desktopEntry and type == ToolsManager.UserEntry ) )
            tools << tool
...
def icon(self, filePath, optionnalFilePath ):
    ...
    if  icon.isNull() and not filePath.isEmpty() :
        icon = mIconProvider.icon( filePath )


    if  icon.isNull() and not optionnalFilePath.isEmpty() :
...
def toolsMenu_triggered(self, action ):
    ...
    elif  workingPath.isEmpty() and QFile.exists( filePath ) :
        ok = QDesktopServices.openUrl( QUrl.fromLocalFile( filePath ) )

    elif  workingPath.isEmpty() :
...
def commandInterpreter(self, command, _arguments, result, interpreter, data ):
    ...
    if  operation == "list" :
        ...
        foreach (  ToolsManager.Tool& tool, manager.mTools )
