...
def startMenuPaths(self):
    ...
    if  SHGetSpecialFolderPathW( NULL, path, CSIDL_COMMON_STARTMENU, False ) :        paths << QString.fromWCharArray( path ).replace( "\\", "/" )

    
    # get start menu files
    if  SHGetSpecialFolderPathW( NULL, path, CSIDL_STARTMENU, False ) :        paths << QString.fromWCharArray( path ).replace( "\\", "/" )
