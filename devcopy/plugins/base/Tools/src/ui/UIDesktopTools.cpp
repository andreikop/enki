...
def closeEvent(self, event ):
    if  isWindowModified(:
        and QMessageBox.question( self, QString.null, tr( "You're about to discard all changes. Are you sure ?" ), QMessageBox.Yes | QMessageBox.No, QMessageBox.No ) == QMessageBox.No )        event.ignore()
        return
...
def applyFilters(self):
    ...
    for item in items:        da = item.data( 0, Qt.UserRole ).value<DesktopApplication*>()
        
        if  not da or mApplications.contains( da.parent.applications.key( *da ) ) :            continue
        ...
        if  visible :            categoriesFilterMatch = False
            
            for filter in categoriesFilter:                if  da.categories.contains( filter, Qt.CaseInsensitive ) :                    categoriesFilterMatch = True
                    break
...
def populateTree(self, _item, folder ):
    ...
    # Folders
    for folderName in folder.folders.keys():        df = &folder.folders[ folderName ]
        item = 0
        
        if  _item :            item = QTreeWidgetItem( _item )
    ...
    # Applications
    for fileName in folder.applications.keys():        da = &folder.applications[ fileName ]
        item = 0
...
def scanApplications(self):
    ...
    foreach (  ToolsManager.Tool& tool, mToolsManager.tools( ToolsManager.DesktopEntry ) )        mApplications << tool.filePath

    
    foreach ( QTreeWidgetItem* item, twLeft.findItems( "*", Qt.MatchWildcard | Qt.MatchRecursive ) )        da = item.data( 0, Qt.UserRole ).value<DesktopApplication*>()
        
        if  not da :            continue

        
        if  mApplications.contains( da.parent.applications.key( *da ) ) :            item.setSelected( True )
...
def on_tbRight_clicked(self):
    for item in twLeft.selectedItems():        da = item.data( 0, Qt.UserRole ).value<DesktopApplication*>()
        
        if  item.isHidden() or not da :            continue
...
def on_tbLeft_clicked(self):
    for item in lwRight.selectedItems():        da = item.data( Qt.UserRole ).value<DesktopApplication*>()
        it = item.data( Qt.UserRole +1 ).value<QTreeWidgetItem*>()
        
        if  it :            mApplications.remove( da.parent.applications.key( *da ) )
    ...
    if  isWindowModified() :        applyFilters()



def on_tbUp_clicked(self):
    if  lwRight.selectedItems().count() > 1 :        QMessageBox.warning( QApplication.activeWindow(), QString.null, tr( "Only one item can be move up, select only one item." ) )
        return

    
    item = lwRight.selectedItems().value( 0 )
    
    if  not item or lwRight.row( item ) == 0 :        return
...
def on_tbDown_clicked(self):
    if  lwRight.selectedItems().count() > 1 :        QMessageBox.warning( QApplication.activeWindow(), QString.null, tr( "Only one item can be move down, select only one item." ) )
        return

    
    item = lwRight.selectedItems().value( 0 )
    
    if  not item or lwRight.row( item ) == lwRight.count() -1 :        return
...
def accept(self):
    if  isWindowModified() :
        ...
        for ( i = 0; i < lwRight.count(); i++ )            item = lwRight.item( i )
