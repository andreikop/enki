...
def startMenuPaths(self):
    ...
    if  xdg_paths.isEmpty() :        xdg_paths << "/usr/share" << "/usr/local/share"

    
    for path in xdg_paths:        paths << QDir.cleanPath( QString( "%1/applications" ).arg( path ) )
...
def scan(self):
    ...
    for menuPath in startMenuPaths():        foreach (  QFileInfo& applicationFile, pMonkeyStudio.getFiles( QDir( menuPath ), "*.desktop" ) )            # get folder object
            df = &mStartMenu
            # get relative path
            ...
            if  not df.applications.contains( applicationFile.absoluteFilePath() :
                and not settings.value( "Name" ).toString().isEmpty() )                # add application
