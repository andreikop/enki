    ...
    while ( not  isLineReady )
        ...
        if  not  isInTemplate :
            if  (newHeader = findHeader(headers)) != NULL:
                ...
                # But do not perform the attachment if the breakClosingHeaderBrackets is setnot 
                ...
                #     should never have a block after itnot 
        ...
        if padOperators:
            if (newHeader = findHeader(operators)) != NULL:
                ...
                # the parenStack must be updated here accordinglynot 
