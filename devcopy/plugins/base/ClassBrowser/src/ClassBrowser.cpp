...
def install(self):
    ...
    # connections
    MonkeyCore.fileManager().documentOpened.connect(self.documentOpened)
    MonkeyCore.fileManager().currentDocumentChanged.connect(self.currentDocumentChanged)
    MonkeyCore.fileManager().opened.connect(self.opened)
    MonkeyCore.fileManager().buffersChanged.connect(self.buffersChanged)
    mDock.browser().entryActivated.connect(self.entryActivated)
    mDock.browser().fileNameActivated.connect(self.fileNameActivated)
    self.propertiesChanged.connect(mDock.browser().setProperties)
    self.integrationModeChanged.connect(MonkeyCore.multiToolBar().notifyChanges)
...
def uninstall(self):
    # disconnections
    disMonkeyCore.fileManager().documentOpened.connect(self.documentOpened)
    disMonkeyCore.fileManager().currentDocumentChanged.connect(self.currentDocumentChanged)
    disMonkeyCore.fileManager().opened.connect(self.opened)
    disMonkeyCore.fileManager().buffersChanged.connect(self.buffersChanged)
    dismDock.browser().entryActivated.connect(self.entryActivated)
    dismDock.browser().fileNameActivated.connect(self.fileNameActivated)
    disself.propertiesChanged.connect(mDock.browser().setProperties)
    disself.integrationModeChanged.connect(MonkeyCore.multiToolBar().notifyChanges)
