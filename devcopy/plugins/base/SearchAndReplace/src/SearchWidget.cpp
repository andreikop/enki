...
#warning QDirModel is deprecated but QCompleter does not yet handle QFileSystemModel - please update when possible.
    ...
    # connections
    connect( cbSearch.lineEdit(), SIGNAL( textEdited(  QString& ) ), self, SLOT( search_textChanged() ) )
...
def setResultsDock(self, dock ):
    ...
    # connections
    connect( mReplaceThread, SIGNAL( resultsHandled(  QString&,  SearchResultsModel.ResultList& ) ), mDock.model(), SLOT( thread_resultsHandled(  QString&,  SearchResultsModel.ResultList& ) ) )
