...
MessageBoxDocks.MessageBoxDocks( QObject* parent )
    ...
    # connections
    connect( mBuildStep.lvBuildSteps, SIGNAL( activated(  QModelIndex& ) ), self, SLOT( lvBuildSteps_activated(  QModelIndex& ) ) )
    connect( mOutput.cbRawCommand.lineEdit(), SIGNAL( returnPressed() ), self, SLOT( cbRawCommand_returnPressed() ) )
    connect( MonkeyCore.consoleManager(), SIGNAL( commandError(  pCommand&, QProcess.ProcessError ) ), self, SLOT( commandError(  pCommand&, QProcess.ProcessError ) ) )
    connect( MonkeyCore.consoleManager(), SIGNAL( commandFinished(  pCommand&, int, QProcess.ExitStatus ) ), self, SLOT( commandFinished(  pCommand&, int, QProcess.ExitStatus ) ) )
    connect( MonkeyCore.consoleManager(), SIGNAL( commandReadyRead(  pCommand&,  QByteArray& ) ), self, SLOT( commandReadyRead(  pCommand&,  QByteArray& ) ) )
    connect( MonkeyCore.consoleManager(), SIGNAL( commandStarted(  pCommand& ) ), self, SLOT( commandStarted(  pCommand& ) ) )
    connect( MonkeyCore.consoleManager(), SIGNAL( commandStateChanged(  pCommand&, QProcess.ProcessState ) ), self, SLOT( commandStateChanged(  pCommand&, QProcess.ProcessState ) ) )
    connect( MonkeyCore.consoleManager(), SIGNAL( commandSkipped(  pCommand& ) ), self, SLOT( commandSkipped(  pCommand& ) ) )
    connect( MonkeyCore.consoleManager(), SIGNAL( newStepAvailable(  pConsoleManagerStep& ) ), self, SLOT( appendStep(  pConsoleManagerStep& ) ) )
    connect( MonkeyCore.consoleManager(), SIGNAL( newStepsAvailable(  pConsoleManagerStepList& ) ), self, SLOT( appendSteps(  pConsoleManagerStepList& ) ) )
