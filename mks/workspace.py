...
class pWorkspace(QFrame):
	...
	def __init__(self, parent):
		...
		self.mOpenedFileExplorer = pOpenedFileExplorer( self )
	...
	def indexOfDocument( self, document ):
		return self.mMdiArea.subWindowList().indexOf( document )

	def documents(self):
		return self.mMdiArea.subWindowList()

	def setCurrentDocument( self, document ):
		if  self.curDocument() != document :
			self.mMdiArea.setActiveSubWindow( document )
	...
	def handleDocument( self, document ):
		...
		self.mMdiArea.blockSignals( True )
		self.mMdiArea.addSubWindow( document )
		self.mMdiArea.blockSignals( False )
	...
	def unhandleDocument( self, document ):
		...
		self.mMdiArea.removeSubWindow( document )
		document.hide()
	...
	def fileSessionRestore_triggered(self):
		# restore files
		for file in mks.monkeycore.settings(monkeycore.settings.value( "Session/Files", [] ).toStringList() ):
			if not self.openFile( file, mks.monkeystudio.defaultCodec() ): # remove it from recents files
				mks.monkeycore.recentsManager().removeRecentFile( file )
		
		# restore projects
		for project in mks.monkeycore.settings(monkeycore.settings.value( "Session/Projects", [] ).toStringList() ):
