...
import enkilib.core.defines
from enkilib.resources.icons import qInitResources, qCleanupResources
...
_CONFIG_PATH = os.path.join(enkilib.core.defines.CONFIG_DIR, 'enki.json')
...
class Core(QObject):
    ...
    **Signal** emitted, when settings dialog had been accepted
    ...
    def init(self, profiler):
        ...
        import enkilib.core.actionmanager
        self._actionManager = enkilib.core.actionmanager.ActionManager(self)
        
        # Imports are here for hack crossimport problem
        import enkilib.core.mainwindow  # pylint: disable=W0621,W0404
        self._mainWindow = enkilib.core.mainwindow.MainWindow()
        ...
        import enkilib.core.uisettings  # pylint: disable=W0404
        self._uiSettingsManager = enkilib.core.uisettings.UISettingsManager()
        if profiler is not None:
            profiler.stepDone('Create UISettings')
        
        import enkilib.core.workspace
        if profiler is not None:
            profiler.stepDone('import workspace')

        self._workspace = enkilib.core.workspace.Workspace(self._mainWindow)
        self._mainWindow.setWorkspace(self._workspace)
        if profiler is not None:
            profiler.stepDone('create workspace')

        import enkilib.core.filefilter
        self._fileFilter = enkilib.core.filefilter.FileFilter()
        if profiler is not None:
            profiler.stepDone('Create FileFilter')
        
        import enkilib.core.locator
        self._locator = enkilib.core.locator.Locator(self._mainWindow)
        if profiler is not None:
    ...
    def mainWindow(self):
        """Get :class:`enkilib.core.mainwindow.MainWindow` instance
        """
        return self._mainWindow

    def actionManager(self):
        """Get main window ::class:`enkilib.core.actionmanager.ActionManager` instance
        """
        return self._actionManager

    def workspace(self):
        """Get :class:`enkilib.core.workspace.Workspace` instance
        """
        return self._workspace

    def config(self):
        """Get :class:`enkilib.core.config.Config` instance
        """
        return self._config
    
    def loadedPlugins(self):
        """Get list of curretly loaded plugins (::class:`enkilib.core.Plugin` instances)
        """
        return self._loadedPlugins
    
    def setIndentHelper(self, language, indentHelper):
        """Set  ::class:`enkilib.core.abstractdocument.IndentHelper` for language. Pass None to clear the value
        """
    ...
    def indentHelper(self, language):
        """Get ::class:`enkilib.core.abstractdocument.IndentHelper` for the language
    ...
    def _loadPlugin(self, name):
        ...
        """
        exec("import enkilib.plugins.%s as module" % name)  # pylint: disable=W0122
    ...
    def _createDefaultConfigFile(self):
        ...
        """
        import enkilib.core.config  # pylint: disable=W0621,W0404
        
        if not os.path.exists(enkilib.core.defines.CONFIG_DIR):
            try:
                os.makedirs(enkilib.core.defines.CONFIG_DIR)
            except (OSError, IOError) as ex:
                raise UserWarning(u'Failed to create directory "%s". Error: %s\n' % \
                                  (enkilib.core.defines.CONFIG_DIR, unicode(str(ex), 'utf8')))
        try:
    ...
    def _createConfig(self):
        ...
        if haveFileInHome:
            try:
                config = enkilib.core.config.Config(True, _CONFIG_PATH)
            except UserWarning:  # messages are shown by the Config class
                pass
        
        # Open default, if previous step failed
        if config is None:
            config = enkilib.core.config.Config(False, _DEFAULT_CONFIG_PATH)
    ...
    def fileFilter(self):
        ...
        See ::mod:`enkilib.core.filefilter`
        """
        return self._fileFilter
    
    def locator(self):
        """::class:`enkilib.core.locator.Locator` instance
    ...
    def uiSettingsManager(self):
        """::class:`enkilib.core.uisettings.UISettingsManager` instance
